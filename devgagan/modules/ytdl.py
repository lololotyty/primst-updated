...
async def process_audio(client, event, url, cookies_env_var=None):
    ...
    random_filename = f"shimperd_pro_{event.sender_id}"
    ...
    try:
        ...
        if os.path.exists(download_path):
            def edit_metadata():
                ...
                audio_file.tags["TPE1"] = TPE1(encoding=3, text="shimperd")
                audio_file.tags["COMM"] = COMM(encoding=3, lang="eng", desc="Comment", text="Processed by shimperd")
...
async def split_and_upload_file(app, sender, file_path, caption):
    ...
    async with aiofiles.open(file_path, mode="rb") as f:
        while True:
            ...
            # Uploading part
            edit = await app.send_message(sender, f"⬆️ Uploading part {part_number + 1}/{total_parts}...")
